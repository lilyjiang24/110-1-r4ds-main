...
```{r}
c(1, 2, 3, 4, 5, 6) # the same as 1:5 # because 6 caomes from length(dataSet)
1:length(dataSet)
```
...
c(0)
```
but a vector of nothing (with type interger)
...
```{r}
dataSet# <- dataSet4
3 checking blocks for dataSet3
```
...
```{r}
# function definition
# two portable programming blacks design for dataSet3
check_class <- function(){...}
check_na <- function(){...}
